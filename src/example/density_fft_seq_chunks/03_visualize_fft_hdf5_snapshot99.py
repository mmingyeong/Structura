...
# @Date: 2025-04-09
# @Filename: 03_visualize_fft_hdf5_snapshot99.py
#
# Description:
#   This script loads a 3D FFT density map from an HDF5 file, computes a 2D projection by summing
...
def main():
    """
    Main function to load a 3D FFT density map from an HDF5 file,
    ...
    # HDF5 file path (3D FFT density map)
    hdf5_path = f"/home/users/mmingyeong/structura/Structura/src/example/density_fft_seq_chunks/final_snapshot-99_fft_density_map_{kernel.__name__}_dx{res}.hdf5"
    if not os.path.exists(hdf5_path):
        logger.error("HDF5 file not found: %s", hdf5_path)
        return
    
    # Candidate keys for the 3D density dataset.
    candidate_keys = ["density", "fft_density", "density_map", "density_fft"]
    ...
    xlabel = "Y (cMpc/h)"
    ylabel = "Z (cMpc/h)"
    title = f"FFT Projection (log10) along {kernel.__name__} Axis 0"
