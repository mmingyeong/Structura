...
# -*- coding: utf-8 -*-
"""
density_calculator_ex_multi.py

Example script for users to compute multiple 3D density maps from particle data
using DensityCalculator. Each combination of kernel and resolution is saved
in a separate HDF5 file. Users can modify input/output paths and parameters.

Author: Mingyeong Yang (for public distribution)
Date: 2025-04-22
"""
...
OUTPUT_FOLDER = "./output_density_maps"
FILE_EXTENSION = ".hdf5"  # or .h5, .npy, etc.
...
logger = logging.getLogger("DensityExampleLogger")
...
def compute_density(args):
    ...
    try:
        ...
        output_dir = os.path.join(OUTPUT_FOLDER, group_key)
        os.makedirs(output_dir, exist_ok=True)
        out_path = os.path.join(output_dir, f"density_{index_str}.hdf5")

        calc = DensityCalculator(particles, GRID_BOUNDS, spacing, use_gpu=False)
        x, y, z, rho = calc.calculate_density_map(kernel_func=kernel, h=BANDWIDTH, use_lut=USE_LOOKUP_TABLE)
        ...
        logger.error(f"❌ Failed to compute/save {kernel.__name__} dx={res:.2f}: {e}")

if __name__ == '__main__':
    files = sorted(f for f in os.listdir(INPUT_FOLDER) if f.endswith(FILE_EXTENSION))
