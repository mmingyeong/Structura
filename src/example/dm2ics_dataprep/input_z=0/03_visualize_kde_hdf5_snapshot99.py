...
HDF5_DIR = "/caefs/data/IllustrisTNG/densitymap-99-dm-hdf5"
...
def visualize_all_conditions():
    ...
    fig.suptitle(f"Log10(Density + {LOG_OFFSET}) | x∈[100,120] cMpc/h | vmin={VMIN}, vmax={VMAX}", fontsize=14)
    output_path = os.path.join(IMG_RESULTS_DIR, f"comparison_2x2_log_offset{LOG_OFFSET}_vmin{VMIN}_vmax{VMAX}.png")
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"[✓] Saved 2x2 comparison: {output_path}")
