...
#resolutions = [0.82, 0.41, 0.164]  # in cMpc/h
resolutions = [0.164]  # in cMpc/h
bandwidth = 2000
use_lut = True
input_folder = "/caefs/data/IllustrisTNG/snapshot-0-ics"
base_output_folder = "/caefs/data/IllustrisTNG/densitymap-ics-hdf5/res_0.16"
# -----------------------------------------------
...
def compute_density(args):
    ...
    try:
        ...
        out_path = os.path.join(output_dir, f"density_snapshot_0_{index_str}.hdf5")
        ...
        if not np.any(rho > 0):
            logger.warning(f"⚠️ Density all zero! kernel={kernel.__name__}, dx={res}, chunk={index_str}")
            with open("logs/fail_chunks.txt", "a") as f:
                f.write(f"{index_str},{kernel_name},dx={res}\n")
            return

        logger.info(f"    → rho stats: min={np.min(rho):.4e}, max={np.max(rho):.4e}, sum={np.sum(rho):.4e}")
